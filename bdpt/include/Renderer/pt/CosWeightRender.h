...
Vec3 cos_weight_render_n(const Ray& r, const std::vector<std::shared_ptr<Object>>& scene, int target_depth) {
    const float epsilon = 1e-3f;
    const int RR = 3;
    Vec3 throughput(1.0f, 1.0f, 1.0f);
    Vec3 radiance(0.0f, 0.0f, 0.0f);
    ...
    for(int bounce = 0; bounce < target_depth; ++bounce) {
