    ...
    for (int path_length = 1; path_length <= max_path_length; ++path_length) {
        ...
        for(int j = image_height - 1; j >= 0; --j) {
            for(int i = 0; i < image_width; ++i) {
                ...
                bdpt_pixels[idx + 0] = static_cast<unsigned char>(ir);
                bdpt_pixels[idx + 1] = static_cast<unsigned char>(ig);
                bdpt_pixels[idx + 2] = static_cast<unsigned char>(ib);
            }
        }

        std::ostringstream bdpt_filename;
        bdpt_filename << path_dir.str() << "/bdpt.png";
        stbi_write_png(bdpt_filename.str().c_str(), image_width, image_height, 3, bdpt_pixels.data(), image_width * 3);

        std::cout << "  Rendering PT (CosWeight)" << std::endl;
        std::vector<unsigned char> pt_pixels(image_width * image_height * 3);
        ...
        for(int j = image_height - 1; j >= 0; --j) {
            for(int i = 0; i < image_width; ++i) {
                ...
                size_t idx = ((image_height - 1 - j) * image_width + i) * 3;
                pt_pixels[idx + 0] = static_cast<unsigned char>(ir);
                pt_pixels[idx + 1] = static_cast<unsigned char>(ig);
                pt_pixels[idx + 2] = static_cast<unsigned char>(ib);
            }
        }

        std::ostringstream pt_filename;
        pt_filename << path_dir.str() << "/cos.png";
        stbi_write_png(pt_filename.str().c_str(), image_width, image_height, 3, pt_pixels.data(), image_width * 3);
